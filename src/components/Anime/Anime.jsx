...
export const Anime = ({ anime,handleBookMark }) => {
//   console.log(anime);
const {cover,title,author,author_img,genre}=anime;
  return (
    <div>
      <div className="card card-side bg-base-100 shadow-sm rounded-4xl">
        <figure className=" rounded-4xl">
          <img className=" w-3xs h-80"
            src={cover}
            alt="Movie"
          />
        </figure>
        <div className="card-body">
          ...
          <ol className="bg-green-300 m-auto flex flex-col rounded-4xl p-5">
            {
                genre.map((g,index)=><li key={index}>{g}</li>)
            }
          </ol>
          <div className="card-actions justify-between items-center">
            <FaBookmark onClick={()=>{
                handleBookMark(anime)
            }} className=" text-cyan-400" size={25}/>
            <button className="btn btn-primary rounded-2xl text-amber-50">Watch</button>
          </div>
        </div>
      </div>
    </div>
